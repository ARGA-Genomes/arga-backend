...
use arga_core::models::{Dataset, Sequence, SequencingEvent, SequencingRunEvent};
...
use crate::matchers::dna_extract_matcher::{self, dna_extract_map, DnaExtractMap, DnaExtractMatch, DnaExtractRecord};
use crate::matchers::sequence_matcher::{self, sequence_map, SequenceMap, SequenceMatch, SequenceRecord};
...
impl From<Record> for DnaExtractRecord {
    fn from(value: Record) -> Self {
        return Self {
            record_id: value.dna_extract_id,
        };
    }
}
...
fn extract_sequencing_events(dataset: &Dataset, records: &MatchedSequences) -> Vec<SequencingEvent> {
    ...
    let sequences = records
        .into_par_iter()
        .map(|(sequence, row)| SequencingEvent {
            id: Uuid::new_v4(),
            dataset_id: dataset.id.clone(),
...
fn extract_sequencing_run_events(records: MatchedSequences, events: &Vec<SequencingEvent>) -> Vec<SequencingRunEvent> {
    info!(total = records.len(), "Extracting sequencing run events");

    let runs = (records, events)
        .into_par_iter()
        .map(|(record, event)| {
        ...
        .flatten()
        .collect::<Vec<SequencingRunEvent>>();
