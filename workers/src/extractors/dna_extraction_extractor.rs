...
use arga_core::models::{Dataset, DnaExtract, DnaExtractionEvent};
...
use crate::matchers::subsample_matcher::{
...
pub fn extract(
    ...
    context: &Vec<Dataset>,
    pool: &mut PgPool,
) -> Result<DnaExtractionExtractIterator, Error> {
...
fn extract_chunk(
    ...
    dataset: &Dataset,
    subsamples: &SubsampleMap,
) -> Result<DnaExtractionExtract, Error> {
...
fn extract_dna_extracts(dataset: &Dataset, records: &MatchedRecords) -> Vec<DnaExtract> {
    ...
    let dna_extracts = records
        .par_iter()
        .map(|(subsample, row)| DnaExtract {
            id: Uuid::new_v4(),
...
fn extract_dna_extraction_events(
    dataset: &Dataset,
    records: MatchedRecords,
    extracts: &Vec<DnaExtract>,
) -> Vec<DnaExtractionEvent> {
    info!(total = records.len(), "Extracting dna extraction events");

    let extractions = (records, extracts)
        .into_par_iter()
        .map(|(record, extract)| {
